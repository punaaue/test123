...
def generate_infinite_stage(stage_idx):
    rng = random.Random(stage_idx) # Seeded random for deterministic enemies
...
def get_enemy_pool(stage):
    ...
    import math
    pool = []
    base_hp = 30 * (1.05 ** stage)
    base_atk = 5 * (1.05 ** stage)
    
    # Boss logic
    if stage > 0 and stage % 10 == 0:
        boss_mult = 1.0 + (stage // 10) * 0.1
        pool.append({
            "name": f"Boss: Stage {stage} Titan", "icon": "👹", "color": "#ff0000",
            "hp": int(base_hp * 3 * boss_mult), "atk": int(base_atk * 2.5 * boss_mult), "exp": 200 + stage*20
        })
        return pool
        
    for _ in range(max(1, min(4, math.ceil(stage/3)))):
        variant = random.choice([
            {"n":"Goblin","i":"👺","c":"#4CAF50","hm":0.8,"am":1.2},
            {"n":"Slime","i":"💧","c":"#2196F3","hm":1.2,"am":0.8},
            {"n":"Skeleton","i":"💀","c":"#9E9E9E","hm":1.0,"am":1.0},
            {"n":"Orc","i":"🐗","c":"#795548","hm":1.5,"am":1.5}
        ])
        pool.append({
            "name": f"Lvl {stage} {variant['n']}", "icon": variant["i"], "color": variant["c"],
            "hp": int(base_hp * variant["hm"]), "atk": int(base_atk * variant["am"]), "exp": 15 + stage*5
        })
    return pool
